...
include Santavalidations
  ...
  scope :include_people, -> {joins(:person).includes(:person)}
  scope :include_santas, -> {includes(:santa).references(:santa)}
  scope :include_partners, -> {includes(:partner).references(:partner)}
